...
단, Dashboard는 예외로 페이지 자체 스크롤을 사용하지 않는다.
...
Dashboard는 사용자가 가장 많이 보는 화면이다.

## 9.1 Desktop Dashboard Layout Rules

Dashboard는 항상 브라우저 한 화면(100vh) 안에서 모두 보여야 한다.
...
좌측 영역은 달력만 배치하며 항상 월간 달력이 한 화면 안에 모두 보이도록 한다.

Desktop 비율은 좌측 72%, 우측 28%를 기본으로 한다.
...
카드의 내용이 많아질 경우 페이지를 늘리지 않고 카드 내부에서만 스크롤한다.

Dashboard 내부에는 새 공고 등록 버튼을 두지 않는다.

새 공고 등록은 Top Navigation 버튼만 사용한다.

지원 통계 카드는 Dashboard에서 제거한다.

Dashboard는 "오늘 무엇을 해야 하는지"를 가장 빠르게 확인할 수 있도록 설계한다.
...
Dashboard는 좌측 달력 72%, 우측 카드 영역 28% 비율을 기준으로 한다.

Dashboard 좌측에는 달력만 배치한다.

Dashboard 우측에는 오늘 일정, 이번 주 일정, TODO 카드만 배치한다.

Dashboard 우측 카드의 내용이 많아질 경우 카드 내부에서만 스크롤한다.
